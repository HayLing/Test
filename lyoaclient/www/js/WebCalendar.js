...
Calendar.prototype.draw = function() {   
  ...
  var mvAry = [];   
  //mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0em;">'; //因 <form> 不能嵌套， 2006-12-01 由寒羽枫改用 Div   
  mvAry[mvAry.length]  = '  <div name="calendarForm" style="margin: 0em;">';   
  mvAry[mvAry.length]  = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:2em;">';   
  mvAry[mvAry.length]  = '      <tr>';   
  mvAry[mvAry.length]  = '        <th align="left" width="1%"><input style="border: 0.0625em solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:1.3em;height:1.5em;';   
  ...
  mvAry[mvAry.length]  = '        <th align="center" width="98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:0.625em;width:5em;height:1.5em;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:0.625em;width:5em;height:1.5em;';   
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  = '"></select></th>';   
  mvAry[mvAry.length]  = '        <th align="right" width="1%"><input style="border: 0.0625em solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:1.3em;height:1.5em;';    
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>';   
  mvAry[mvAry.length]  = '      </tr>';   
  mvAry[mvAry.length]  = '    </table>';   
  mvAry[mvAry.length]  = '    <table id="calendarTable" width="100%" style="border:0em solid #CCCCCC;background-color:#FFFFFF;font-size:2em;';   
...
//2009-03-03 添加的代码，放置时间的行   
  ...
  mvAry[mvAry.length]  = '      <select name="calendarHour" id="calendarHour"  style="font-size:0.75em;width:3.75em;"></select>' + Calendar.language["hour"][this.lang];   
  mvAry[mvAry.length]  = '<span style="'  
    if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时时隐藏“分”   
  mvAry[mvAry.length]  = '"><select name="calendarMinute" id="calendarMinute" style="font-size:0.75em;width:3.75em;"></select>' + Calendar.language["minute"][this.lang]+'</span>';   
    mvAry[mvAry.length]  = '<span style="'  
    if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时、分时隐藏“秒”   
   mvAry[mvAry.length]  = '"><select name="calendarSecond" id="calendarSecond" style="font-size:0.75em;width:2.625em;"></select>'+ Calendar.language["second"][this.lang]+'</span>';   
  mvAry[mvAry.length]  = '      </td></tr>';   
     
  mvAry[mvAry.length]  = '    </table>';   
  //mvAry[mvAry.length]  = '  </from>';   
  mvAry[mvAry.length]  = '      <div align="center" style="padding:0.25em 0.25em 0.25em 0.25em;background-color:' + calendar.colors["input_bg"] + ';">';   
  mvAry[mvAry.length]  = '        <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 0.0625em solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width: 4.375em; height: 2em; font-size: 1.25em;cursor:pointer;"/>';   
  ...
  mvAry[mvAry.length]  = '" style="border: 0.0625em solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:4.375em;height:2em;font-size:1.25em;cursor:pointer"/>';   
  mvAry[mvAry.length]  = '        <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 0.0625em solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width: 4.375em; height: 2em; font-size: 1.25em;cursor:pointer"/>';   
...
Calendar.prototype.show = function (dateObj, popControl) {   
  ...
  if(dateObj.id == "end_time"){
  	popControl = document.getElementById("start_time");
  }
  var xy = this.getAbsPoint(popControl);
  var wid = document.body.clientWidth;
  var dm = screen.width;
  //this.panel.style.left = xy.x/16 + "em";
  //this.panel.style.top = ((xy.y + dateObj.offsetHeight)/16) + "em";
  this.panel.style.left = (wid - (dm/320))/2/16 + '%';;//((xy.x + 20) /16) + "em"; //由寒羽枫 2007-02-11 修改 → 加入自定义偏移量
  this.panel.style.top = (((xy.y +25) + topY + dateObj.offsetHeight)/16) + "em";
...
{   
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 0.0625em solid #CCCCCC;width:20.9375em;font-size:0.75em;"></div>';   
    if(document.all)   
    {   
...
//2009-03-03 添加的代码，放置时间的行
  mvAry[mvAry.length]  = '      <tr style="';
    if(calendar.DateMode >= pickMode["day"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到时日隐藏“时间”
    mvAry[mvAry.length]  = '"><td align="center" colspan="7">';
  mvAry[mvAry.length]  = '      <select name="calendarHour" id="calendarHour" style="font-size:0.625em;width:3.75em;"></select>' + Calendar.language["hour"][this.lang];
  mvAry[mvAry.length]  = '<span style="'
    if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时时隐藏“分”
  mvAry[mvAry.length]  = '"><select name="calendarMinute" id="calendarMinute" style="font-size:0.625em;width:3.75em;"></select>' + Calendar.language["minute"][this.lang]+'</span>';
    mvAry[mvAry.length]  = '<span style="'
    if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时、分时隐藏“秒”
   mvAry[mvAry.length]  = '"><select name="calendarSecond" id="calendarSecond" style="font-size:0.75em;width:2.625em;"></select>'+ Calendar.language["second"][this.lang]+'</span>';
  mvAry[mvAry.length]  = '      </td></tr>';
...
Calendar.prototype.show = function (dateObj, popControl) {
 ...
 this.bindSecond();
  ...
  var dm = screen.width;
  this.panel.style.left = (wid - (dm/320))/2/16 + '%';//((xy.x + 20) /16) + "em"; //由寒羽枫 2007-02-11 修改 → 加入自定义偏移量
  this.panel.style.top = (((xy.y +20) + topY + dateObj.offsetHeight)/16) + "em";
